...
# Usage: .\deploy_to_ec2.ps1 -EC2_IP "YOUR_EC2_PUBLIC_IP"
...
try {
    ...
    # 2. Create deployment package (exclude unnecessary files)
    ...
    if (Test-Path "firebase-key.json") {
        ...
        Write-Host "⚠️  WARNING: firebase-key.json not found!" -ForegroundColor Yellow
    }
...
'@
    ...
    ssh -i $KEY_FILE -o StrictHostKeyChecking=no ubuntu@$EC2_IP $DEPLOY_COMMANDS
    ...
    Write-Host "   1. Update your Flutter app's api_config.dart:" -ForegroundColor White
    Write-Host "      static const String _prodUrl = `"http://$EC2_IP:8000`";" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "   2. Or build with:" -ForegroundColor White
    Write-Host "      flutter build apk --dart-define=BASE_URL=http://$EC2_IP:8000" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "   3. View logs:" -ForegroundColor White
    Write-Host "      ssh -i $KEY_FILE ubuntu@$EC2_IP 'cd ~/brick-bhatta-backend && docker-compose logs -f'" -ForegroundColor Yellow
    Write-Host ""
